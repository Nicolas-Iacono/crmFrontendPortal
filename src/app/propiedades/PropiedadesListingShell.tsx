...
import type { FiltrosDisponibles, PageResponse, PropiedadLista } from "@/lib/api";
...
export default function PropiedadesListingShell({
  filtros,
  cleanParams,
  propiedades,
  headingTitle,
  ...
  propiedades: PageResponse<PropiedadLista>;
  headingTitle?: string;
  ...
  const defaultTitle = cleanParams.usuarioId
    ? inmobiliariaNombre || "Inmobiliaria"
    : "Propiedades exclusivas";

  const defaultSubtitle = cleanParams.usuarioId ? (
    <>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-10 gap-4">
          <div className="min-w-0">
            <h1 className="font-headline font-extrabold text-3xl sm:text-4xl text-on-surface tracking-tight mb-2 flex items-center gap-3">
              ...
              {headingTitle ?? defaultTitle}
            </h1>
            <p className="text-on-surface-variant font-body">{headingSubtitle ?? defaultSubtitle}</p>
            {cleanParams.usuarioId && (
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <Link href="/propiedades" className="text-sm font-bold text-primary hover:underline">
                  Ver todo el portal
                </Link>
              </div>
            )}
        ...
        {totalPages > 1 && (
          <div className="mt-16 flex justify-center">
            <div className="inline-flex items-center gap-1 sm:gap-2 bg-surface-container-low p-2 rounded-full flex-wrap justify-center max-w-full">
              {currentPage > 0 && (
                <Link
                  href={getListingPathForQueryString({
                    ...cleanParams,
